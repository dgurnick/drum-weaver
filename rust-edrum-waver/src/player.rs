...
pub fn play_combined(
    track_source: Amplify<Decoder<BufReader<File>>>,
    click_source: Amplify<Decoder<BufReader<File>>>,
    device: &Device,
) -> Result<(), String> {
    let (_stream, stream_handle) = OutputStream::try_from_device(&device)
        .map_err(|e| format!("Failed to create track output stream: {}", e))?;
...
pub fn play_separate(
    ...
    let (_track_stream, track_stream_handle) = OutputStream::try_from_device(track_device)
        .map_err(|e| format!("Failed to create track output stream: {}", e))?;
    let (_click_stream, click_stream_handle) = OutputStream::try_from_device(click_device)
        .map_err(|e| format!("Failed to create track output stream: {}", e))?;

    let track_sink = Sink::try_new(&track_stream_handle)
        .map_err(|e| format!("Failed to create audio sink: {}", e))?;
    let click_sink = Sink::try_new(&click_stream_handle)
        .map_err(|e| format!("Failed to create audio sink: {}", e))?;
...
pub fn run_cli(arguments: PlayerArguments) -> Result<(), String> {
    ...
    let (track_path_str, click_path_str) =
        get_file_paths(&arguments.music_folder, arguments.track_position);

    let track =
        File::open(track_path_str).map_err(|e| format!("Failed to open track file: {}", e))?;
    let click =
        File::open(click_path_str).map_err(|e| format!("Failed to open click file: {}", e))?;

    let track_source = Decoder::new(BufReader::new(track))
        .map_err(|e| format!("Failed to decode track file: {}", e))?;
    let click_source = Decoder::new(BufReader::new(click))
        .map_err(|e| format!("Failed to decode click file: {}", e))?;
